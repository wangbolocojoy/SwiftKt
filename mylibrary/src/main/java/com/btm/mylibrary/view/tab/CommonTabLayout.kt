...
import com.btm.mylibrary.view.tab.utils.FragmentChangeManager
import com.btm.mylibrary.view.tab.utils.UnreadMsgUtils
...
class CommonTabLayout @JvmOverloads constructor(
    ...
    private fun obtainAttributes(
        ...
        mTextBold = ta.getInt(
            R.styleable.CommonTabLayout_tl_textBold,
            TEXT_BOLD_NONE
        )
    ...
    private fun addTab(position: Int, tabView: View) {
        ...
        val iv_tab_icon =
            tabView.findViewById<View>(R.id.iv_tab_icon) as ImageView
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    override fun onAnimationUpdate(animation: ValueAnimator) {
        ...
        if (mIndicatorWidth < 0) {   //indicatorWidth小于0时,原jpardogo's PagerSlidingTabStrip
            ...
            val indicatorLeft =
                p.left + (currentTabView.width - mIndicatorWidth) / 2
            mIndicatorRect.left = indicatorLeft.toInt()
            mIndicatorRect.right = (mIndicatorRect.left + mIndicatorWidth).toInt()
        }
        invalidate()
    ...
    fun setMsgMargin(
        ...
        if (tipView != null) {
            ...
            if (mIconVisible) {
                if (iconH <= 0) {
                    iconH = mContext.resources
                        .getDrawable(mTabEntitys[position].tabSelectedIcon).intrinsicHeight
                        .toFloat()
                }
                margin = mIconMargin
            }
            if (mIconGravity == Gravity.TOP || mIconGravity == Gravity.BOTTOM) {
                ...
                lp.topMargin =
                    ...
                    ) else dp2px(bottomPadding as Float)
            } else {
                lp.leftMargin = dp2px(leftPadding)
                lp.topMargin = if (mHeight > 0) (mHeight - Math.max(
                    textHeight,
                    iconH
                )).toInt() / 2 - dp2px(bottomPadding as Float) else dp2px(bottomPadding as Float)
            }
